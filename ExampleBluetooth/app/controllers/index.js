...
function onClickOpen(e){
	if(btc.isAvailable()){ // xác định thiết bị có được tích hợp bluetooth không
		if(btc.isEnabled()){// Xác định thiết bị đã mở hay chưa
			alert("Bluetooth close ");
			btc.closeBluetooth();
			$.btn_open.title="On";
...
$.btn_open.addEventListener("click", onClickOpen);
